...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget listItem(int index, List<SectionModel> cartList) {
    ...
    double price= 0;
    cartList[index].finalPrice != "0" ?
     price = double.parse(cartList[index].finalPrice.toString())
    ...
    if (price == 0)
      ...
      //         .price!)) -
      //     price;
      offPer = double.parse(cartList[index].discount);
      // offPer = (off *
      //         100 /
                        ...
                        children: <Widget>[
                          Row(
                            ...
                            children: [
                             ...
                             // cartList[index].finalPrice.toString() == "0" ?
                              GestureDetector(
                                child: Padding(
                                  padding: const EdgeInsetsDirectional.only(
                            ...
                            children: <Widget>[
                              ...
                              Text(
                                CUR_CURRENCY! + " " + price.toString(),
                                    //cartList[index].finalPrice.toString(),
                                style: TextStyle(
                                    color:
                                        Theme.of(context).colorScheme.fontColor,
                                    fontWeight: FontWeight.bold),
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    ...
    double price = double.parse(cartList[index].finalPrice);
       // cartList[index].productList![0].prVarientList![selectedPos].disPrice!);
    if (price == 0)
      price = double.parse(cartList[index].finalPrice);
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          deliveryCharge = double.parse(getdata['delivery_charge']);
  ...
  Future<void> _sendRequest() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        Response response =
        await post(sendUserRequest, body: parameter, headers: headers)
            .timeout(Duration(seconds: timeOut));
                ...
                children: <Widget>[
                  ...
                  Container(
                    child: Column(mainAxisSize: MainAxisSize.min, children: <
                      ...
                      SimBtn(
                          size: 0.9,
                          title: cartList[0].finalPrice == "0" ? "Send Request" : "Checkout",
                         // getTranslated(context, 'PROCEED_CHECKOUT'),
                          onBtnSelected: () async {
                            print("this is final price ${cartList[0].finalPrice}");
                             if(cartList[0].finalPrice == "0"){
                               _sendRequest();
                             }else {
                               if (isOnOff == true) {
                                 if (oriPrice > 0) {
